...
Write-Host ""
Write-Host "1. Testing Azure API..." -ForegroundColor Yellow

try {
    $response = Invoke-WebRequest -Uri "$ApiUrl/api/productos" -Method Get -ErrorAction Stop
    if ($response.StatusCode -eq 200) {
        Write-Host "[OK] API is responding correctly (HTTP 200)" -ForegroundColor Green
    }
}
catch {
...
try {
    ...
    if ($response.StatusCode -eq 200) {
        Write-Host "[OK] Swagger UI is available (HTTP 200)" -ForegroundColor Green
        Write-Host "     Access at: $ApiUrl/swagger" -ForegroundColor Cyan
    }
}
catch {
    Write-Host "[ERROR] Swagger error: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    if ($response.StatusCode -eq 200) {
        Write-Host "[OK] Frontend is deployed (HTTP 200)" -ForegroundColor Green
        Write-Host "     Access at: $FrontendUrl" -ForegroundColor Cyan
    }
}
catch {
    Write-Host "[ERROR] Frontend error: $($_.Exception.Message)" -ForegroundColor Red
}

# Test 4: API Products Endpoint
Write-Host ""
Write-Host "4. Testing Products Endpoint..." -ForegroundColor Yellow

try {
    $response = Invoke-WebRequest -Uri "$ApiUrl/api/productos" -Method Get -ContentType "application/json" -ErrorAction Stop
    $products = $response.Content | ConvertFrom-Json
    Write-Host "[OK] Products loaded successfully" -ForegroundColor Green
    Write-Host "     Total products: $($products.Count)" -ForegroundColor Cyan
}
catch {
    Write-Host "[ERROR] Products error: $($_.Exception.Message)" -ForegroundColor Red
}
